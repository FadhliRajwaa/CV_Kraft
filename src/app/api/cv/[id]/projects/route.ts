...
import { cvProjectsSchema } from "@/lib/validations/cv";
...
export async function PATCH(request: Request, { params }: RouteContext): Promise<NextResponse> {
  ...
  const parsed = cvProjectsSchema.safeParse(body);
  ...
  const newData = { ...currentData, projects: parsed.data.projects };
  ...
  return NextResponse.json({ message: "Proyek berhasil disimpan" }, { status: 200 });
}

...
import { cvSummarySchema } from "@/lib/validations/cv";
...
export async function PATCH(request: Request, { params }: RouteContext): Promise<NextResponse> {
  ...
  const parsed = cvSummarySchema.safeParse(body);
  ...
  const newData = { ...currentData, summary: parsed.data.summary };
  ...
  return NextResponse.json({ message: "Ringkasan berhasil disimpan" }, { status: 200 });
}

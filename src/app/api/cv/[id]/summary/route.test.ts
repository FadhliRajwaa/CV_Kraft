...
function createRequest(body: Record<string, unknown>): Request {
  return new Request(`http://localhost/api/cv/${CV_ID}/summary`, {
    method: "PATCH",
...
describe("PATCH /api/cv/[id]/summary", () => {
  beforeEach(() => {
  ...
  it("mengembalikan 403 jika CV bukan milik user", async () => {
    ...
    const response = await PATCH(createRequest({ summary: "Ringkasan baru" }), ROUTE_CONTEXT);
    const payload = await response.json();
  ...
  it("mengembalikan 400 jika payload tidak valid", async () => {
    ...
    const response = await PATCH(createRequest({ summary: "" }), ROUTE_CONTEXT);
  ...
  it("mengupdate CV.data.summary jika payload valid", async () => {
    mockAuthenticated();
    findCvMock.mockResolvedValueOnce({
      userId: USER_ID,
      data: {
        personalInfo: { fullName: "Fadhli" },
      },
    ...
    const response = await PATCH(
      createRequest({ summary: "Software engineer dengan pengalaman 5 tahun." }),
      ROUTE_CONTEXT,
    );
    const payload = await response.json();

    expect(response.status).toBe(200);
    expect(payload.message).toBe("Ringkasan berhasil disimpan");
    ...
    expect(updateCvMock.mock.calls[0]?.[0]).toMatchObject({
      ...
      data: {
        data: {
          personalInfo: { fullName: "Fadhli" },
          summary: "Software engineer dengan pengalaman 5 tahun.",
        },
      },
    });
  });

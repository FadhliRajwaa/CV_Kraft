...
import { cvCertificationsSchema } from "@/lib/validations/cv";
...
export async function PATCH(request: Request, { params }: RouteContext): Promise<NextResponse> {
  ...
  const parsed = cvCertificationsSchema.safeParse(body);
  ...
  const newData = { ...currentData, certifications: parsed.data.certifications };
  ...
  return NextResponse.json({ message: "Sertifikasi berhasil disimpan" }, { status: 200 });
}

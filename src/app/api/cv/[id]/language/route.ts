...
import { cvLanguageSchema } from "@/lib/validations/cv";
...
export async function PATCH(request: Request, { params }: RouteContext): Promise<NextResponse> {
  ...
  const parsed = cvLanguageSchema.safeParse(body);
  ...
  return NextResponse.json({ message: "Bahasa CV berhasil disimpan" }, { status: 200 });
}

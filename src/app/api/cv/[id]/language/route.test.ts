...
function createRequest(body: Record<string, unknown>): Request {
  return new Request(`http://localhost/api/cv/${CV_ID}/language`, {
    method: "PATCH",
...
describe("PATCH /api/cv/[id]/language", () => {
  beforeEach(() => {
  ...
  it("mengembalikan 401 jika tidak terautentikasi", async () => {
    ...
    const response = await PATCH(createRequest({ language: "id" }), ROUTE_CONTEXT);
  ...
  it("mengembalikan 403 jika CV bukan milik user", async () => {
    mockAuthenticated();
    findCvMock.mockResolvedValueOnce({ userId: "user-2" });

    const response = await PATCH(createRequest({ language: "en" }), ROUTE_CONTEXT);
    const payload = await response.json();
  ...
  it("mengembalikan 400 jika payload tidak valid", async () => {
    ...
    const response = await PATCH(createRequest({ language: "jp" }), ROUTE_CONTEXT);
  ...
  it("mengupdate CV.language jika payload valid", async () => {
    mockAuthenticated();
    ...
    expect(payload.message).toBe("Bahasa CV berhasil disimpan");

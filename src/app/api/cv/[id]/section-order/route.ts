...
import { cvSectionOrderSchema } from "@/lib/validations/cv";
...
export async function PATCH(request: Request, { params }: RouteContext): Promise<NextResponse> {
  ...
  const parsed = cvSectionOrderSchema.safeParse(body);
  ...
  const newData = { ...currentData, sectionOrder: parsed.data.sectionOrder };
  ...
  return NextResponse.json({ message: "Urutan section berhasil disimpan" }, { status: 200 });
}

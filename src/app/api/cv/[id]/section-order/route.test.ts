...
function createRequest(body: Record<string, unknown>): Request {
  return new Request(`http://localhost/api/cv/${CV_ID}/section-order`, {
    method: "PATCH",
...
describe("PATCH /api/cv/[id]/section-order", () => {
  beforeEach(() => {
  ...
  it("mengupdate CV.data.sectionOrder jika payload valid", async () => {
    mockAuthenticated();
    ...
    expect(payload.message).toBe("Urutan section berhasil disimpan");
    ...
    expect(updateCvMock.mock.calls[0]?.[0]).toMatchObject({
      ...
      data: {
        data: {
          summary: "existing",
          sectionOrder: VALID_PAYLOAD.sectionOrder,
        },
      },
    });
  });

...
function createRequest(body: Record<string, unknown>): Request {
  return new Request(`http://localhost/api/cv/${CV_ID}/personal-info`, {
    method: "PATCH",
...
describe("PATCH /api/cv/[id]/personal-info", () => {
  beforeEach(() => {
  ...
  it("mengembalikan 400 jika payload tidak valid", async () => {
    ...
    const response = await PATCH(
      createRequest({ fullName: "", email: "invalid", phone: "" }),
      ROUTE_CONTEXT,
    );
  ...
  it("mengupdate CV.data.personalInfo jika payload valid", async () => {
    mockAuthenticated();
    findCvMock.mockResolvedValueOnce({ userId: USER_ID, data: { summary: "existing" } });
    ...
    expect(payload.message).toBe("Data pribadi berhasil disimpan");

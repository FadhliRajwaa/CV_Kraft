...
function createRequest(body: Record<string, unknown>): Request {
  return new Request(`http://localhost/api/cv/${CV_ID}/educations`, {
    method: "PATCH",
...
describe("PATCH /api/cv/[id]/educations", () => {
  beforeEach(() => {
  ...
  it("mengupdate CV.data.educations jika payload valid", async () => {
    mockAuthenticated();
    ...
    expect(payload.message).toBe("Pendidikan berhasil disimpan");
    ...
    expect(updateCvMock.mock.calls[0]?.[0]).toMatchObject({
      ...
      data: {
        data: {
          summary: "existing",
          educations: VALID_PAYLOAD.educations,
        },
      },
    });
  });
